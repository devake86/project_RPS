"""
project_RPS - game_menu_sneak_a_peek
"""
...
from .game_loop_sneak_a_peek_quiz_cache_clear import game_loop_sneak_a_peek_quiz_cache_clear_inmenu

# menu sneak a peek
def game_menu_sneak_a_peek():
    ...
    print(f"     ")
    print(f"     ")
    print(f"     [0] CLEAR ALL RIGHT ANSWERS FROM QUIZ CACHE")
    print(f"     ")
    print(f"     ")
    print(f"     ")
    print(f"     ")
    ...
    while True:
        ...
        # check if input is length of not 1 or value is not 1, 2, ..., 5, 6 or 0
        if len(input_lernfeld_de) !=1 or input_lernfeld_de not in ("1","2","3","4","5","6","0"):
            sys.stdout.write("\033[F")
            sys.stdout.write("\033[2K")
            sys.stdout.flush()
            continue

        if int(input_lernfeld_de) == 0:
            ...
            print(f"     ")
            print(f"     ")
            print(f"     All RIGHT answers have been cleared from quiz cache.")
            print(f"     ")
            print(f"     ")
            print(f"     ")
            continue
